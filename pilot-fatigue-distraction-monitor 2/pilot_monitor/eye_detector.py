...
from pilot_monitor.utils import resize
...
class EyeDetector:
    ...
    @staticmethod
    def _calc_1eye_vector(landmarks, eye_lms_nums, eye_iris_num, frame_size):
        """Return signed, eye-box-normalized (dx, dy) iris displacement.

        Unlike ``_calc_1eye_score`` (an undirected magnitude), this keeps the
        sign of each axis so horizontal ("looking left/right") and vertical
        ("looking up/down") deviation can be told apart. New to this project:
        the base project only ever computed the undirected magnitude.
        """
        iris = landmarks[eye_iris_num, :2] * frame_size
        ...
        eye_width = (eye_x_max - eye_x_min) * frame_size[0]
        eye_height = (eye_y_max - eye_y_min) * frame_size[1]
        if eye_width <= np.finfo(float).eps or eye_height <= np.finfo(float).eps:
            return None, None
    ...
    def get_gaze_vector(self, landmarks, frame_size):
        """Return signed, mean (dx, dy) gaze deviation across both eyes.

        ``dx`` > 0 means the iris sits toward the outer/right side of the eye
        box; ``dy`` > 0 means it sits low in the eye box. Returns
        ``(None, None)`` if neither eye produced a usable measurement.
        """
        left = self._calc_1eye_vector(
            landmarks, self.EYES_LMS_NUMS[:6], self.LEFT_IRIS_NUM, frame_size
        )
        right = self._calc_1eye_vector(
            landmarks, self.EYES_LMS_NUMS[6:], self.RIGHT_IRIS_NUM, frame_size
        )
        pairs = [pair for pair in (left, right) if pair[0] is not None]
        if not pairs:
            return None, None
        dx = float(np.mean([pair[0] for pair in pairs]))
        dy = float(np.mean([pair[1] for pair in pairs]))
        return dx, dy
