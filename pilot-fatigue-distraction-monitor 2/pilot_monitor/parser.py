...
def get_args(argv=None):
    ...
    input_group.add_argument(
        ...
        type=Path,
        default=None,
        help="Path to a recorded video file to replay instead of a live camera "
        "(runs the identical pipeline, for reproducible evaluation)",
    )
    input_group.add_argument(
        "--camera-params",
        "--camera_params",
    ...
    input_group.add_argument(
        ...
        type=str,
        default=None,
        help="Pilot identifier used to load a calibration profile from "
        "--profiles-dir. Without one, the system falls back to uncalibrated "
        "(non-personalized) operation.",
    )
    input_group.add_argument(
        "--profiles-dir",
        type=Path,
        default=Path("profiles"),
        help="Directory holding per-pilot calibration profiles and trained "
        "temporal models (default: profiles/)",
    )
    input_group.add_argument(
        "--log-path",
        type=Path,
        default=None,
        help="Write structured per-frame research logs (CSV) to this path",
    )
    input_group.add_argument(
        "--no-gru",
        dest="use_gru",
        action="store_false",
        default=True,
        help="Disable the per-pilot GRU temporal anomaly model even if a trained one exists",
    )
