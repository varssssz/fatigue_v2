...
def get_landmarks(lms, return_raw=False):
    """Convert MediaPipe landmarks to NumPy and return the largest valid face.

    Parameters
    ----------
    return_raw : bool
        When True, also return the same face's landmarks *before* clipping
        to [0, 1]. The observation-quality layer (``pilot_monitor.quality``)
        needs the unclipped coordinates to detect how much of the face fell
        outside the frame; clipping (needed by every downstream geometric
        computation) would otherwise hide that signal.
    """
    surface = 0
    biggest_face = None
    biggest_face_raw = None
    for lms0 in lms:
        ...
        if new_surface > surface:
            ...
            biggest_face_raw = raw
            surface = new_surface

    if surface == 0:
        raise ValueError("No valid face landmarks were provided")
    if return_raw:
        return biggest_face, biggest_face_raw
    return biggest_face
