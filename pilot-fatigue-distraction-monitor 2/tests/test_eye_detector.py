...
def test_gaze_vector_is_signed_and_matches_magnitude():
    detector = EyeDetector()
    frame_size = (640, 480)
    landmarks = make_landmarks(frame_size)

    dx, dy = detector.get_gaze_vector(landmarks, frame_size)
    magnitude = detector.get_Gaze_Score(None, landmarks, frame_size)

    # Both left and right iris sit to the right of their eye-box center in
    # this fixture (see make_landmarks), so the averaged dx should be > 0.
    assert dx > 0
    assert (dx**2 + dy**2) ** 0.5 == pytest.approx(magnitude, rel=1e-6)
